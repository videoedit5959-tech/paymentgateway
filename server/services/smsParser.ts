...
export class BkashSmsParser {
  public static canHandle(message: string): boolean {
    return /bKash|TrxID/i.test(message);
  }
  ...
  public static parse(message: string): Partial<ParsedSms> | null {
    ...
    const amountMatch =
      text.match(/(?:received(?: payment)?|Cash In|Tk|BDT)\s*(?:Tk|BDT)?\s*([0-9,]+(?:\.[0-9]{1,2})?)/i) ||
      text.match(/([0-9,]+(?:\.[0-9]{1,2})?)\s*Tk/i);

    const amount = amountMatch ? parseFloat(amountMatch[1].replace(/,/g, '')) : 0;

    // Match Balance: "Balance Tk 42,500.00" or "Balance : Tk 42,500.00"
    const balanceMatch = text.match(/Balance\s*[:]?\s*(?:Tk|BDT)?\s*([0-9,]+(?:\.[0-9]{1,2})?)/i);
    const balance = balanceMatch ? parseFloat(balanceMatch[1].replace(/,/g, '')) : undefined;

    // Match Fee: "Fee Tk 0.00" or "Fee: Tk 5.00"
    const feeMatch = text.match(/Fee\s*[:]?\s*(?:Tk|BDT)?\s*([0-9,]+(?:\.[0-9]{1,2})?)/i);
    ...
    const senderMatch = text.match(/from\s*[:]?\s*(01[3-9][0-9]{8})/i);
...
export class NagadSmsParser {
  public static canHandle(message: string): boolean {
    return /Nagad|TxnID/i.test(message);
  }
  ...
  public static parse(message: string): Partial<ParsedSms> | null {
    ...
    const amountMatch =
      text.match(/Amount\s*[:]?\s*(?:Tk|BDT)?\s*([0-9,]+(?:\.[0-9]{1,2})?)/i) ||
      text.match(/(?:Money Received|Payment Received)[^0-9]*([0-9,]+(?:\.[0-9]{1,2})?)/i) ||
      text.match(/(?:Tk|BDT)\s*([0-9,]+(?:\.[0-9]{1,2})?)/i);
    ...
    const senderMatch = text.match(/(?:Sender|Customer|from)\s*[:]?\s*(01[3-9][0-9]{8})/i);
    const sender = senderMatch ? senderMatch[1] : undefined;
...
export class SmsParserEngine {
  ...
  public static parse(message: string, deviceId: string = ''): ParsedSms {
    ...
    const genericTrx = message.match(/(?:TrxID|TxnID)[:\s]+([A-Z0-9]{6,16})/i);
    const genericAmount = message.match(/(?:Tk|BDT)[:\s]*([0-9,]+(?:\.[0-9]{1,2})?)/i);

    if (genericTrx && genericAmount) {
      const prov: MfsProvider = /Nagad/i.test(message) ? 'NAGAD' : 'BKASH';
